...
void NIfunction(int, void*)
{
	...
	if (g_c8BitDepth.at<uchar>(i, j) >=1.0*3500/7000*255 )
	{
...
double yellowde(Mat asrc){
	...
	for (int i = 0; i < num_row; i += 2)
	{
		...
		//	uchar*outData = mask.ptr<uchar>(i);
		for (int j = 0; j < num_col; j += 2){
			if ((curr_r_hue[j] <= 25 && curr_r_hue[j] >= 14) && (curr_r_satur[j]>80 && curr_r_satur[j] < 255))
			{
				count_yellow++; count++;
			}
			else  count++;
	...
	for (int unsigned i = 0; i < contours.size(); i++)
	{
			...
			((center[i].x + radius[i]) <srcImage.cols) && ((center[i].y + radius[i]) < srcImage.rows) && (radius[i]>10))
		{
			...
			if (a == 1)
			{
				...
				if (((red >= index0) && (red>0.45) && (blue<0.05) && (yellow<0.11)) || ((blue >= index1) && (blue>0.17) && (red<0.25))) {
					index0 = red;
					index1 = blue; 
					index2 = yellow;
					flag = i;
				...
				if (((blue >= index1) && (blue<0.20) && (blue>0.02) && (red >= index0) && ((red>0.1)) && (red<0.45)) || ((yellow >= index2 && (yellow>0.06) && (red >= index0) && (red>0.1) && (red<0.55) && (blue<0.03)))){
					index0 = red;
					index1 = blue ;
					index2 = yellow;
					flag = i;
