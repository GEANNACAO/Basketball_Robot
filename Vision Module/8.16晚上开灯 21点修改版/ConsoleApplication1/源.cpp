...
#ifdef _debug
	imwrite("depth.jpg", g_c8BitDepth);
	...
	for (int unsigned i = 0; i < contours.size(); i++)
	{
			...
			((center[i].x + radius[i]) <srcImage.cols) && ((center[i].y + radius[i]) < srcImage.rows) && (radius[i]>10))
		{
			...
			if (a == 1)
			{
				...
				if (((red >= index0) && (red>0.37) && (blue<0.1) && (yellow<0.1)) || ((blue >= index1) && (blue>0.09) && (yellow>0.04) && (red<0.35))){
					index0 = red;
					index1 = blue; 
					index2 = yellow;
					flag = i;
				...
				if (((blue >= index1) && (blue<0.54) && (blue>0.02) && (red >= index0) && (red<0.42) && (yellow<0.04)) || ((yellow >= index2 && (yellow > 0.04) && (red >= index0) && (red > 0.1) && (red < 0.43) && (blue<0.07)))){
					index0 = red;
					index1 = blue;
					index2 = yellow;
					flag = i;
