...
void NIfunction(int, void*)
{
	...
	{
		if ((g_c8BitDepth.at<uchar>(150, j) < (3500.0 / 7000 * 255)) && (g_c8BitDepth.at<uchar>(150, j) > (1000.0 / 7000 * 255)))
		{
			if (x<g_c8BitDepth.at<uchar>(150, j))
				x = g_c8BitDepth.at<uchar>(150, j);
		}

	}
	for (int j = 50; j < 400; j++)
	{
		if ((g_c8BitDepth.at<uchar>(100, j) < (3500.0 / 7000 * 255)) && (g_c8BitDepth.at<uchar>(100, j) > (1000.0 / 7000 * 255)))
		{
			if (x<g_c8BitDepth.at<uchar>(100, j))
				x = g_c8BitDepth.at<uchar>(100, j);
		}

	}
	for (int i = 0; i < g_c8BitDepth.rows; i++){
		for (int j = 0; j < g_c8BitDepth.cols; j++)
		if ((g_c8BitDepth.at<uchar>(i, j) >= (x - 30)) || (i<200))
		{
...
void getAreaContours(vector<vector<Point>>&contours)
{
	...
	int amax = 22000;// 最大轮廓面积,80cm-350cm,18000
...
double yellowde(Mat asrc){
	...
	for (int i = 0; i < num_row; i += 2)
	{
		...
		//	uchar*outData = mask.ptr<uchar>(i);
		for (int j = 0; j < num_col; j += 2){
			if ((curr_r_hue[j] <= 34 && curr_r_hue[j] >= 16) && (curr_r_satur[j]>80 && curr_r_satur[j] < 255)&& (curr_r_value[j]>46 && curr_r_value[j]<200))
			{
				count_yellow++; count++;
			}
			else  count++;
	...
	for (int unsigned i = 0; i < contours.size(); i++)
	{
			...
			((center[i].x + radius[i]) <srcImage.cols) && ((center[i].y + radius[i]) < srcImage.rows) && (radius[i]>10))
		{
			...
			if (a == 1)
			{
				...
				if (((red >= index0) && (red>0.40) && (blue<0.1) && (yellow<0.1)) || ((blue >= index1) && (blue>0.09) && (yellow>0.037) && (red<0.35))){
					index0 = red;
					index1 = blue; 
					index2 = yellow;
					flag = i;
				...
				if (((blue >= index1) && (blue<0.29) && (blue>0.02) && (red >= index0) && (red<0.42) && (yellow<0.04)) || ((yellow >= index2 && (yellow > 0.04) && (red >= index0) && (red > 0.1) && (red < 0.43) && (blue<0.05)))){
					index0 = red;
					index1 = blue;
					index2 = yellow;
					flag = i;
...
int main()
{
	...
	//	if (inputData[0] == '1' || inputData[0] == '3' )
		com_data = '3';
