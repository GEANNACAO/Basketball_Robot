...
void NIfunction(int, void*)
{
	...
	if (g_c8BitDepth.at<uchar>(i, j) >= x-5)
	{
...
double yellowde(Mat asrc){
	...
	for (int i = 0; i < num_row; i += 2)
	{
		...
		//	uchar*outData = mask.ptr<uchar>(i);
		for (int j = 0; j < num_col; j += 2){
			if ((curr_r_hue[j] <= 22 && curr_r_hue[j] >= 16) && (curr_r_satur[j]>77 && curr_r_satur[j] < 255))
			{
				count_yellow++; count++;
			}
			else  count++;
...
double redde(Mat asrc){
	...
	for (int i = 0; i < num_row; i += 2)
	{
		...
		//	uchar*outData = mask.ptr<uchar>(i);
		for (int j = 0; j < num_col; j += 2){
			if ((curr_r_hue[j] <= 11 && curr_r_hue[j] >= 0) && (curr_r_satur[j]>60 && curr_r_satur[j]<255) && (curr_r_value[j]>46 && curr_r_value[j]<200))//找颜色
			{
				count_red1++; count++;
			}
...
void houghcircle(Mat asrc, Mat depth, int data)
{
	int a = data;

	Mat midImage = depth ;
    Mat dstImage=depth;//临时变量和目标图的定义
	Mat element = getStructuringElement(MORPH_RECT,Size(5,5));
	morphologyEx(midImage, midImage, MORPH_CLOSE, element);
	//imshow("闭运算",midImage);

	//【3】转为灰度图并进行图像平滑
//	cvtColor(depth, midImage, CV_BGR2GRAY);//转化边缘检测后的图为灰度图
//	GaussianBlur(midImage, midImage, Size(5, 5), 2, 2);

	//【4】进行霍夫圆变换
	vector<Vec3f> circles;
	HoughCircles(midImage, circles, CV_HOUGH_GRADIENT, 1.5, 100, 36, 30, 0, 0);
	Mat imgROI;
	//【5】依次在图中绘制出圆
	...
	for (size_t i = 0; i < circles.size(); i++)
	{
		...
		//绘制圆心
	//	circle(depth, center, 3, Scalar(0, 255, 0), -1, 8, 0);
		//绘制圆轮廓
	//	circle(depth, center, radius, Scalar(155, 50, 255), 3, 8, 0);
		if (((((circles[i][0]) - radius) > 0) && (((circles[i][0]) + radius)<640)) &&
			((((circles[i][1]) - radius)>0) && (((circles[i][1]) + radius) < 480)) && (radius>10))
		{
			imgROI = asrc(Rect(((cvRound(circles[i][0])) - radius), (cvRound((circles[i][1]) - radius)), 2 * radius, 2 * radius));
			...
			if (a == 5)
			{
				...
				blue = bluede(imgROI);
				red = redde(imgROI);
				if ((blue >= index) && ((blue<0.13)) && (blue>0.02) && (red >= temp) && ((red > 0.1)) && (red<0.35)) {
					index = blue;
					temp = red;
					j = i;
				}
	...
	{
#ifdef _debug
		cout << j << "球" << circles[j][0] << endl;
		cout << "球r  " << circles[j][2] << endl;
		printf("%.2d\n", g_c8BitDepth.at<uchar>(circles[j][1], circles[j][0]));
#endif
		cout << a << "球找到" << endl;
		cout << j << "球" << circles[j][0] << endl;
		cout << j << "球" << circles[j][1] << endl;
		point = circles[j][0];
		juli = 1.0*g_c8BitDepth.at<uchar>(circles[j][1], circles[j][0]) / 255 * 7000;
		cout << juli << endl;
	}
	if (j == -1)
		cout << a << "球未找到" << endl;
...
void picprocess(Mat asrc, Mat depth, int data)
{
	...
	if (j>-1)
	{
		...
		cout << j << "球" << center[j].x << endl;	
		point = center[j].x;
		juli = 1.0*g_c8BitDepth.at<uchar>(center[j].y, center[j].x)/255*7000;
		cout << juli << endl;
	}
	if(j==-1)
		cout << a << "球未找到" << endl;
...
int main()
{
	...
	while (1)
	{
		...
		if (inputData[0] == '1' || inputData[0] == '2' || inputData[0] == '3' || inputData[0] == '4' || inputData[0] == '5' || inputData[0] == '6' || inputData[0] == '7' || inputData[0] == '8')
	...
	//	cout << inputData[0] << endl;
		...
		case '4':
			...
			break;
		case '5':
			houghcircle(img, g_c8BitDepth, 5);
			break;
		case '6':
			houghcircle(img, g_c8BitDepth, 6);
			break;
		case '7':
			houghcircle(img, g_c8BitDepth, 7);
			break;
		case '8':
			houghcircle(img, g_c8BitDepth, 8);
			break;
		default:
			break;
		}
