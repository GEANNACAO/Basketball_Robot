...
void getAreaContours(vector<vector<Point>>&contours)
{
	...
	int amax = 18000;// 最大轮廓面积,80cm-350cm,18000
	...
	for (int unsigned i = 0; i < contours.size(); i++)
	{
			...
			((center[i].x + radius[i]) <srcImage.cols) && ((center[i].y + radius[i]) < srcImage.rows) && (radius[i]>10))
		{
			...
			if (a == 1)
			{
				...
				per = 1.0*yellow / red;
				cout << per << endl;
				if (((red >= index0) && (red>0.40) && (blue<0.05) && (yellow<0.025)) || ((blue >= index1) && (blue>0.098) && (red<0.35) && (per>0.20) && (per<0.8)) || (blue >= index1) && (blue>0.30)) {
					index0 = red;
					index1 = blue; 
					index2 = yellow;
					flag = i;
				...
				per = 1.0*yellow / red;
				cout<<per << endl;
				if (((blue >= index1) && (blue<0.27) && (blue>0.02) && (red >= index0) && (red<0.52) && (per<0.16)) || ((yellow >= index2 && (yellow>0.04) && (red >= index0) && (red>0.1) && (red<0.55) && (blue<0.03)))){
					index0 = red;
					index1 = blue ;
					index2 = yellow;
					flag = i;
